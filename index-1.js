...
app.post("/ask-ai", verifyAuth, async (req, res) => {
  try {
    ...
    // 0.4) Deposit/withdrawal/balance/VIP/referral queries — same
    // deterministic-DB-lookup approach as match-earnings, koi AI call nahi.
    ...
    // 0.5) Deterministic FAQ match — agar question kisi active FAQ se
    // near-exact match karta hai, LLM provider ko call kiye bina hi seedha
    ...
    try {
      ...
      console.log("⚠️ tryAnswerFromFaqDeterministic failed, falling back to AI:", faqDetErr.message);
      // fall through to normal AI flow below
    }
    ...
    const systemPrompt =
      ...
      "You must answer ONLY using the FAQ list below, which the app admin has set up with " +
      "fixed answers. This is your one and only knowledge source — you have no other " +
      ...
      "How to respond:\n" +
      "1) MATCH: If the user's question clearly matches one FAQ (even if it's phrased " +
      ...
      "3) NO MATCH: If the question is not covered by any FAQ above, or is unrelated to the " +
      "app, politely say (in the user's language per the LANGUAGE RULE above) you're not able " +
      "to help with that specific query and tell the user to contact Customer Support from the " +
      ...
      "include this token for case 1 or case 2 replies.\n" +
...
app.post("/tickets/get-upload-signature", verifyAuth, async (req, res) => {
  try {
    ...
    // Only these params go into the signature — the client must send back
    // the exact same values when it POSTs to Cloudinary's upload endpoint.
    // max_bytes is included so Cloudinary itself rejects an oversized file
    // even if a client skips the client-side size check.
    const paramsToSign = { timestamp, folder, max_bytes: maxBytes };
