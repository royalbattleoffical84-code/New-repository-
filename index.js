...
async function callAiWithRetry(availableProviders, systemPrompt, conversationMessages, maxRetries = 2) {
  ...
  for (let i = 0; i <= maxRetries; i++) {
    ...
    lastAttempt = attempt;
    if (i < maxRetries) {
      console.log(`⏳ AI call attempt ${i + 1} failed (transient), retrying in ${(i + 1) * 1500}ms...`);
      await new Promise((r) => setTimeout(r, (i + 1) * 1500)); // 1.5s, then 3s
    }
  }
  return lastAttempt;
}
