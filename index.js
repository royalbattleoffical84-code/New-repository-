...
app.post("/ai-admin-parse", verifyAuth, async (req, res) => {
  try {
    ...
    const systemPrompt =
      ...
      "English visual description for generating a banner image for this match. If the admin " +
      "described ANY specific visual detail for the banner/thumbnail themselves (colors, " +
      "characters, objects, mood, weapons, background, etc.) -- even briefly, even in Hindi/" +
      "Hinglish -- you MUST translate/incorporate those EXACT details into thumbnailPrompt, " +
      "not a generic substitute. Only invent a fully generic prompt (like 'Free Fire squad " +
      "battle royale, fiery orange and blue action poster, dramatic soldiers') when the admin " +
      "gave NO visual details at all for the banner.) }\n" +
    ...
    let summaryOut = String(parsed.summary || "").slice(0, 500);
    if (previewThumbnailUrl) {
      summaryOut += " (Preview banner is a quick draft -- the real banner generated on Confirm will look different/better, same theme.)";
    }
