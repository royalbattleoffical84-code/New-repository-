...
admin.initializeApp({
  ...
  databaseURL: "https://new-project-f17be-default-rtdb.firebaseio.com",
});
...
app.post("/request-withdrawal", verifyAuth, async (req, res) => {
  try {
    const uid = req.uid;
    const { wdType, amount, accountInfo } = req.body || {};
    ...
    const tx = await uRef.transaction((u) => {
      ...
      if (uWin < amt) { insufficientFunds = true; return; }
      u.winningCash = uWin - amt;
      return u;
    });

    if (!tx.committed) {
      return res.status(400).json({ error: insufficientFunds ? "Insufficient Balance" : "Wallet update failed, try again" });
    }
    ...
    console.log(`✅ Withdrawal requested: uid=${uid}, type=${wdType}, amount=${amt}`);
    return res.json({ success: true, withdrawalId: pushRef.key });
...
app.post("/credit-deposit", verifyAuth, async (req, res) => {
  try {
    ...
    console.log(`✅ Deposit credited: uid=${uid}, orderId=${orderId}, amount=${amount}`);
    return res.json({ success: true, amount });
...
app.post("/ask-ai", verifyAuth, async (req, res) => {
  try {
    ...
    // stats questions ke liye), koi AI call nahi.
    ...
    const systemPrompt =
      ...
      "- Home: Main dashboard, quick access to all features.\n" +
      "- Earn: Ways to earn coins/rewards (daily tasks, referrals, etc.).\n" +
      "- Wallet: Check balance, deposit money, request withdrawal.\n" +
      "- Profile: User account details and personal info.\n" +
      "- Leaderboard: Rank list of top players.\n" +
      ...
      "- Support: Raise a complaint or support ticket.\n" +
    ...
    try {
      ...
      if (firstAttempt.ok) {
        ...
        const secondAttempt = await callLeastBusy(availableProviders, systemPrompt, conversationMessages, excluded);
...
app.post("/ai-parse-match", verifyAuth, async (req, res) => {
  try {
    ...
    if (!attempt.ok) {
      attempt = await callLeastBusy(availableProviders, systemPrompt, conversationMessages);
    }
    if (!attempt.ok) {
...
const AI_ADMIN_ACTION_TYPES = ["create_coupon", "create_redeem_code", "add_faq", "edit_faq", "reply_ticket", "close_ticket", "create_match", "cancel_match"];
...
app.post("/ai-admin-parse", verifyAuth, async (req, res) => {
  try {
    ...
    const systemPrompt =
      ...
      "this list (e.g. deposits, settings, banning users, cancelling a match WITH refund -- " +
      "refunds must always be done manually, never via this assistant), respond with " +
      'actionType "unsupported" and explain briefly in the "summary" field why -- NEVER invent ' +
      "an actionType not in the list above.\n\n" +
    ...
    if (parsed.actionType === "cancel_match") {
    ...
    // create_match ke liye Pollinations.ai se ek preview thumbnail URL bhi
    // bana dete hain (free, no API key) taaki admin confirm karne se pehle
    // dekh sake kaisa banner lagega. Ye sirf ek URL hai (koi extra request
    // nahi karte yahan) -- browser jab <img> tag me isko load karega tabhi
    // Pollinations image generate karega.
...
app.post("/ai-admin-execute", verifyAuth, async (req, res) => {
  try {
    ...
    if (actionType === "create_match") {
      ...
      // Banner: agar AI ne thumbnailPrompt diya tha, wahi Pollinations URL
      // seedha bannerUrl ke roop me save kar dete hain -- ye ek live-generated
      // image URL hai (Pollinations har request par fresh image deta hai,
      // par yehi URL dobara hit hone par wahi image consistently deta hai).
      // Admin baad me Match Setup se ise kabhi bhi replace kar sakta hai.
      let bannerUrl = null;
      if (p.thumbnailPrompt) {
        bannerUrl = `https://image.pollinations.ai/prompt/${encodeURIComponent(p.thumbnailPrompt)}?width=1200&height=675&nologo=true`;
      }
