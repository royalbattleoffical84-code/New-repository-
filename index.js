...
app.post("/tickets/get-upload-signature", verifyAuth, async (req, res) => {
  try {
    ...
    // TEMP DEBUG: "Invalid Signature" errors ko debug karne ke liye —
    // Render Logs me confirm karega ki live server pe exact kaunse
    // Cloudinary credentials load hue hain, bina full secret expose kiye
    // (sirf uski length aur pehle/aakhri 2 characters dikhate hain).
    const secretVal = process.env.CLOUDINARY_API_SECRET || "";
    console.log("🔍 [DEBUG] Cloudinary config in use:", {
      cloudName: process.env.CLOUDINARY_CLOUD_NAME,
      apiKey: process.env.CLOUDINARY_API_KEY,
      secretLength: secretVal.length,
      secretPreview: secretVal.length > 4 ? `${secretVal.slice(0,2)}...${secretVal.slice(-2)}` : "(too short/missing)",
      paramsToSign,
